...
import com.sparkutils.shim.{Id, expressions}
 ...
 */
case class ExpressionLookup(attributesUsed: expressions.Names.Identifiers = Set.empty, unknownSparkFunctions: expressions.Names.Identifiers = Set.empty, lambdas: Set[Id] = Set.empty, sparkFunctions: Set[String] = Set.empty)
...
object VariablesLookup {
   ...
   * @return
   */
  def fieldsFromExpression(expr: Expression, knownLambdaLookups: ProcessedLambdas = Map.empty): expressions.ExpressionLookup = {
    def children(res: expressions.ExpressionLookup, children: Seq[Expression]): expressions.ExpressionLookup =
      children.foldLeft(res){
        (curRes, exp) =>
          accumulate(curRes, exp)
      }

    def accumulate(res: expressions.ExpressionLookup, exp: Expression): expressions.ExpressionLookup =
      exp match {
        // unresolved case where we cannot see more unresolved functions
    ...
    val ids = accumulate(expressions.ExpressionLookup(), expr)
