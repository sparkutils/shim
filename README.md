...
Shim is a thin facade over Apache Spark internal APIs smoothing over differences between runtime versions.  The official [documentation site can be found here](https://sparkutils.github.io/shim/).
...
Please see [the changelog](https://sparkutils.github.io/shim/latest/background/changelog/) for full details.

Shim 0.3.0 is fully AgnosticEncoder based for Spark 4.x encoding and all encoding related functions expecting ExpressionEncoder will throw NotImplementedError.

All end of service Databricks runtimes and OSS Spark versions have been removed but a compatibility layer for codegen differences between Databricks and OSS versions has been added along with support for custom BeanEncoding and a tryCastCompat function. 
...
is built using a 3.5 base but specifically targeting a 14.3 Databricks runtime (e.g. for the StaticInvoke change).  Developers therefore use an appropriate OSS runtime and sparkCompatVersion to build against but users may choose other runtime versions.

Non OSS users should use the "compilation" artifacts to build against as provided scope and "runtime" as compile, with the "compilation" artefact higher on the classpath (see below compilation section)
