...
object UnresolvedFunction5 {
  ...
  def apply(
    ...
    ignoreNulls: Boolean = false) = UnresolvedFunction(nameParts.split("."), arguments, isDistinct, filter)
  ...
  def unapply(unresolvedFunction: UnresolvedFunction): Option[(String, Seq[Expression], Boolean, Option[Expression], Boolean)] =
    unresolvedFunction match {
      case UnresolvedFunction(_, argumentExpressions, is, filter, ignoreNulls) =>
        Some((Names.toName(unresolvedFunction), argumentExpressions, is, filter, ignoreNulls))
      case _ => None
    }
...
object UnresolvedFunction4 {
  ...
  def unapply(unresolvedFunction: UnresolvedFunction): Option[(String, Seq[Expression], Boolean, Option[Expression])] =
    unresolvedFunction match {
      case UnresolvedFunction(_, argumentExpressions, is, filter, _) =>
        Some((Names.toName(unresolvedFunction), argumentExpressions, is, filter))
      case _ => None
    }
