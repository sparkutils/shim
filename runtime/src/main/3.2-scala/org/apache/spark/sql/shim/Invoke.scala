...
object Invoke5 {
  ...
  def unapply(cast: Invoke): Option[(Expression,String,DataType,Seq[Expression],Seq[AbstractDataType])] =
    cast match {
      case Invoke(targetObject, functionName, dataType, arguments, methodInputTypes, propagateNull, returnNullable) =>
...
object Invoke8 {
  ...
  def unapply(cast: Invoke): Option[(Expression,String,DataType,Seq[Expression],Seq[AbstractDataType],Boolean,Boolean,Boolean)] =
    cast match {
      case Invoke(targetObject, functionName, dataType, arguments, methodInputTypes, propagateNull, returnNullable) =>
        Some((targetObject, functionName, dataType, arguments, methodInputTypes, propagateNull, returnNullable, false))
      case _ => None
    }
}
