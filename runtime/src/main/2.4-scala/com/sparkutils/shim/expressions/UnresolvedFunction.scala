...
object UnresolvedFunction5 {
  ...
  def apply(
    ...
    ignoreNulls: Boolean = false) = UnresolvedFunction(FunctionIdentifier(nameParts), arguments, isDistinct)
  ...
  def unapply(unresolvedFunction: UnresolvedFunction): Option[(String, Seq[Expression], Boolean, Option[Expression], Boolean)] =
    unresolvedFunction match {
      case UnresolvedFunction(_, argumentExpressions, is) =>
        Some((Names.toName(unresolvedFunction), argumentExpressions, is, None, false))
      case _ => None
    }
...
object UnresolvedFunction4 {
  ...
  def apply(
             ...
             arguments: Seq[Expression],
             isDistinct: Boolean,
             filter: Option[Expression] = None) = UnresolvedFunction(FunctionIdentifier(nameParts), arguments, isDistinct)

  def unapply(unresolvedFunction: UnresolvedFunction): Option[(String, Seq[Expression], Boolean, Option[Expression])] =
    unresolvedFunction match {
      case UnresolvedFunction(_, argumentExpressions, is) =>
        Some((Names.toName(unresolvedFunction), argumentExpressions, is, None))
      case _ => None
    }
